...
for a = a_var
    ...
    [roots_2, ~, exitflag] = fsolve(@(x)stst_trace(x,a), roots_2);
    Roots_vec_2(veci, :) = roots_2;
    veci = veci + 1;
end

plot(a_var, Roots_vec_1(:,2))
hold on
plot(a_var, Roots_vec_2(:,2))
xlabel('$\alpha$')
ylabel('$v_4/\beta_{osc}$')
%xlim(0,0.8)
%legend('1', '2')
set(gca, "FontSize",16)
exportgraphics(gca,'Trace_Steady_States_Graph_fsolve_v4_alpha.png','Resolution',300)
