...
%plot(a_var,cs_1_Fvec) %I wanted to plot the values of alpha against one set of bifurcation points.
%hold on
plot(a_var,cs_2_Fvec)
hold on
plot(a_var,cs_1_Tvec) %I wanted to plot the values of alpha against one set of bifurcation points.
hold on
plot(a_var,cs_2_Tvec)
hold on
yline(0,'-k')
xlabel('$\alpha$')
ylabel('$c$')
%ylim([-0.1 0.4])
legend({'F', 'Trace 1','Trace 2'}, 'Location', 'northeast')
set(gca, "FontSize", 16)
exportgraphics(gca,'Trace_Steady_States_Graph_c_alpha_fzero_0177_zoom.png','Resolution',300)
